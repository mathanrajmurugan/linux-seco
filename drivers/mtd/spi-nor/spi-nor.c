...
#define SST_WRITE_PAGE		BIT(18)	/* use SST PAGE byte programming */
#define SST_ERASE		BIT(19) /* use SST PAGE byte programming */
};
...
static int spi_nor_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	/* whole-chip erase? */
	if ((len == mtd->size && !(nor->flags & SNOR_F_NO_OP_CHIP_ERASE)) || nor->info->flags & SST_ERASE ) {
...
static const struct flash_info spi_nor_ids[] = {
	...
	{ "mx25u6435f",  INFO(0xc22537, 0, 64 * 1024, 128, SECT_4K | SPI_NOR_QUAD_READ | SPI_NOR_SKIP_SFDP) },
	...
	{ "sst26vf032b", INFO(0xbf2642, 0, 64 * 1024, 64, SECT_4K | SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ | SST_WRITE_PAGE | RESET_MEMORY | SST_BLOCK_PROTECT | SST_ERASE) },
...
{
	int ret, val;
	printk("%s %d : Start function to enable qe bit bit(6) \n", __func__,__LINE__);
			       ...
			       struct spi_nor_flash_parameter *params)
{
	...
	if (params->hwcaps.mask & (SNOR_HWCAPS_READ_QUAD |
				   SNOR_HWCAPS_PP_QUAD | SNOR_HWCAPS_READ_1_1_4)) {
		switch (JEDEC_MFR(info)) {
		case SNOR_MFR_MACRONIX:
			params->quad_enable = macronix_quad_enable;
			break;
		 ...
		 const struct spi_nor_hwcaps *hwcaps)
{
	...
	if ( JEDEC_MFR(info) == SNOR_MFR_SST &&
		...
		dev_info(nor->dev," sst: reset memory\n");
	...
	else if (info->flags & SST_WRITE_PAGE)

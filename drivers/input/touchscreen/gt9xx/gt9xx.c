		  ...
		  u8 *key_value)
{
	...
	for (i = 0; i < touch_num; i++) {
		...
		if ( ts->pdata->reverse_xy ) 
			GTP_REVERSE_XY(ts->pdata, points[i].x, points[i].y);
		else {
			if ( ts->pdata->reverse_x )
    			GTP_REVERSE_X(ts->pdata, points[i].x);
			if ( ts->pdata->reverse_y ) 
    			GTP_REVERSE_Y(ts->pdata, points[i].y);
		}
...
void gtp_int_output(struct goodix_ts_data *ts, int level)
{
	...
	if (level == 0) {
		if (ts->pinctrl.pinctrl &&  ts->pinctrl.int_out_low != NULL)
		...
		if (ts->pinctrl.pinctrl &&  ts->pinctrl.int_out_high != NULL) {
			pinctrl_select_state(ts->pinctrl.pinctrl,
...
void gtp_int_sync(struct goodix_ts_data *ts, s32 ms)
{
	...
	if (ts->pinctrl.pinctrl && ts->pinctrl.int_input != NULL ) {
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	usleep_range(25000, 30000);		/*  T4: > 5ms */
...
static int gtp_config_read_proc (struct seq_file *seq, void *offset) {
...
static int gtp_config_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, gtp_config_read_proc,  PDE_DATA(inode));
}

static const struct file_operations config_proc_ops = {
	.owner = THIS_MODULE,
	.open = gtp_config_open_fs,
        .read = seq_read,
...
s32 gtp_get_fw_info(struct i2c_client *client, struct goodix_fw_info *fw_info)
{
	...
	if ( data->pdata->sensor_id < 0 ) {
		...
		fw_info->sensor_id = data->pdata->sensor_id;
	}
	return ret;
}
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	of_property_read_u32(np, "goodix,ts-reverse-xy", &pdata->reverse_xy);

	of_property_read_u32(np, "goodix,ts-reverse-x", &pdata->reverse_x);

	of_property_read_u32(np, "goodix,ts-reverse-y", &pdata->reverse_y);

	of_property_read_u32(np, "goodix,ts-hasBtnStylus", &pdata->hasBtnStylus);
	...
	of_property_read_u32(np, "goodix,sensor-id", &pdata->sensor_id);
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pdata->reverse_xy = false;
	pdata->reverse_x = false;
	pdata->reverse_y =false;
	pdata->sensor_id = -1;
#endif
...
static int __init gtp_init(void)
{
    ...
    if ( !goodix_wq )   {
		printk(KERN_ERR "Creat workqueue failed.");  
		return -ENOMEM;
    }

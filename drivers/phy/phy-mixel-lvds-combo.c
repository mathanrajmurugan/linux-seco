...
static int mixel_lvds_combo_phy_init(struct phy *phy)
{
	...
	if(lvds_phy->en_custom_bpp_cfg){
		val |= lvds_phy->bpp_cfg;
	}
	else{
		val |= BIT24;
	}
	...
	printk("phy-mixel-lvds-compo bpp register actual value = 0x%x",val = phy_csr_read(phy, DPI) );
	mutex_unlock(&lvds_phy->lock);
	clk_disable_unprepare(lvds_phy->phy_clk);
...
static int mixel_lvds_combo_phy_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(dev->of_node, "en_custom_bpp_cfg")) {

		lvds_phy->en_custom_bpp_cfg = 1;
		if (lvds_phy->en_custom_bpp_cfg)
			printk("phy-mixel-lvds-compo enabled custom bpp configuration \n");

		if (of_property_read_bool(dev->of_node, "16bpp_bit16_cfg1")) {
			lvds_phy->bpp_cfg = BIT16_CFG1;
			printk("phy-mixel-lvds-compo bpp configuration = 16bpp_bit16_cfg1\n");
		}
		if (of_property_read_bool(dev->of_node, "16bpp_bit16_cfg2")) {
			lvds_phy->bpp_cfg = BIT16_CFG2;
			printk("phy-mixel-lvds-compo bpp configuration = 16bpp_bit16_cfg2\n");
		}
		if (of_property_read_bool(dev->of_node, "16bpp_bit16_cfg3")) {
			lvds_phy->bpp_cfg = BIT16_CFG3;
			printk("phy-mixel-lvds-compo bpp configuration = 16bpp_bit16_cfg3\n");
		}
		if (of_property_read_bool(dev->of_node, "18bpp_bit18_cfg1")) {
			lvds_phy->bpp_cfg = BIT18_CFG1;
			printk("phy-mixel-lvds-compo bpp configuration = 18bpp_bit18_cfg1\n");
		}
		if (of_property_read_bool(dev->of_node, "18bpp_bit18_cfg2")) {
			lvds_phy->bpp_cfg = BIT18_CFG2;
			printk("phy-mixel-lvds-compo bpp configuration = 18bpp_bit18_cfg2\n");
		}
		if (of_property_read_bool(dev->of_node, "24bpp_bit24")) {
			lvds_phy->bpp_cfg = BIT24;
			printk("phy-mixel-lvds-compo bpp configuration = 24bpp_bit24\n");
		}
	}else{
		printk("phy-mixel-lvds-compo custom bpp configuration is not enabled\n");
	}
